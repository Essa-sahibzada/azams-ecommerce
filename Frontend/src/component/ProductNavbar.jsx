...
const ProductNavbar = () => {
  const { cartItems } = useCart(); // ✅ cart → cartItems fix
      ...
      >
        <div className="container mx-auto px-6 lg:px-12 h-[68px] flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            <Link
              to="/login"
              className="hidden md:block text-[9px] tracking-[2.5px] uppercase text-[#8C8478] hover:text-[#1A1A18] transition-colors border-r border-[#E2DDD5] pr-6"
            >
              Login
            </Link>
            ...
            <Link to="/cart" className="relative group flex items-center gap-1">
              <span className="text-[9px] tracking-[2.5px] uppercase text-[#1A1A18] group-hover:text-[#C9A96E] transition-colors">
                Bag
              </span>
              <span
                className="text-[9px] tracking-[1px] transition-colors"
        ...
        {/* ── Mobile Menu ── */}
        <div className={`md:hidden overflow-hidden transition-all duration-300 bg-[#F7F4EF] border-t border-[#E2DDD5] ${menuOpen ? 'max-h-64 py-6' : 'max-h-0'}`}>
